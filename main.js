...
const translations = {
    es: {
        ...
        instructions: "Usa el Mouse para mirar • Scroll o Menú superior para navegar • Clic en los objetos 3D",
        modal_back: "Volver",
        ...
        about_desc: "Mi objetivo es formar parte de un equipo artístico en las áreas de cine, animación y publicidad. Mi experiencia es una mezcla de trabajo creativo, diseño gráfico y producción audiovisual, reforzada por mi facilidad para trabajar en equipo y mi compromiso firme por entregar trabajo de calidad.",
    ...
    en: {
        ...
        about_desc: "My goal is to be part of an artistic team in the fields of film, animation and advertising. My experience blends creative work, graphic design and audiovisual production, backed by my ease working in a team and my firm commitment to delivering quality work.",
...
const loaderLangBtn = document.getElementById('loader-lang-btn');

function syncLangButtons() {
    const label = currentLang.toUpperCase();
    langToggleBtn.innerText = label;
...
const tracks = [
    ...
    { name: "Mondamusic Afrobeat afro beat", src: "assets/music/02-Mondamusic Afrobeat afro beat.mp3" },
...
const masterAudioBtn = document.getElementById('master-audio-btn');
const prevTrackBtn = document.getElementById('prev-track');
const nextTrackBtn = document.getElementById('next-track');
const trackNameDisplay = document.getElementById('track-name-display');
const musicVolumeSlider = document.getElementById('volume-slider-music');
const sfxVolumeSlider = document.getElementById('volume-slider-sfx');
const sfxToggle = document.getElementById('sfx-toggle');

audioMenuToggle.addEventListener('click', (e) => { e.stopPropagation(); audioDropdown.classList.toggle('active'); });
...
masterAudioBtn.addEventListener('click', () => {
    ...
    const t = translations[currentLang];
    masterAudioBtn.innerText = isAudioPlaying ? t.audio_pause : t.audio_play;
...
function playCurrentTrack() {
    ...
    currentAudioElement.onerror = () => {
        console.warn('[Audio] No se pudo reproducir "' + track.name + '" → ' + track.src + '. Revisa que el archivo exista con ese nombre EXACTO (mayusculas/minusculas incluidas) en assets/music/. Saltando a la siguiente pista...');
        ...
        if (isAudioPlaying && consecutivePlaybackErrors < tracks.length) {
            ...
            console.warn('[Audio] Ninguna pista de la playlist pudo reproducirse. Revisa la carpeta assets/music/ en el hosting.');
        }
    };
}
...
const SFX_PRESETS = {
    click: { start: 720, end: 240, dur: 0.11, wave: 'sine', noise: true, noiseFreq: 3200, level: 1.0 },
    nav:   { start: 520, end: 780, dur: 0.13, wave: 'triangle', noise: false, level: 0.9 },
    open:  { start: 300, end: 900, dur: 0.22, wave: 'sine', noise: true, noiseFreq: 1800, level: 1.0 },
...
function playSFX(presetName) {
    ...
    osc.start(); osc.stop(audioCtx.currentTime + p.dur);
...
const interval = setInterval(() => {
    progress += 25;
    if (progress >= 100) { clearInterval(interval); bar.style.width = '100%'; startBtn.style.opacity = '1'; }
    else bar.style.width = progress + '%';
}, 100);

let wantsMusicOnStart = true;
const musicChoiceBtn = document.getElementById('music-choice-btn');
const musicChoiceLabel = document.getElementById('music-choice-label');
...
function updateScrollHintText() {
    scrollHintTextEl.innerText = scrollHintTextForSection(currentSectionIndex);
}
...
// ==========================================================================
function createGalaxyBackgroundTexture(colors) {
...
function createCrystalStarTexture() {
    ...
    const spike = (len, width) => {
        ctx.beginPath();
        ctx.moveTo(0, -len); ctx.lineTo(width, 0); ctx.lineTo(0, len); ctx.lineTo(-width, 0);
...
function buildStarfield() {
    ...
    const mat = new THREE.PointsMaterial({
        color: 0xffffff, size: 1.6, sizeAttenuation: true, transparent: true,
        opacity: 0.8, depthWrite: false, map: STAR_TEXTURES.soft,
...
function createNebulaSpriteTexture(colorA) {
    const size = 256;
    const c = document.createElement('canvas');
    c.width = size; c.height = size;
    const ctx = c.getContext('2d');
...
function buildNebulaClouds(specs) {
    const list = specs || [
...
const BACKGROUND_PRESETS = [
    {
        ...
        fogColor: 0x0d0c1c, fogDensity: 0.075,
        nebula: [
            { pos: [-45, 22, -6], color: 'rgba(130,70,210,0.30)', size: 95 },
            { pos: [50, -8, -38], color: 'rgba(0,200,220,0.20)', size: 115 },
            { pos: [-35, 12, -72], color: 'rgba(255,0,120,0.15)', size: 100 },
        ...
        starColor: 0xffffff, starOpacity: 0.8, starShape: 'soft'
    },
    {
        id: 'azul-noche',
        ...
        fogColor: 0x050b24, fogDensity: 0.09,
        nebula: [
        ...
        starColor: 0xbfd4ff, starOpacity: 0.75, starShape: 'soft'
    },
    {
        id: 'aurora',
        label: 'Aurora Boreal',
        bg: ['#0a2a2c', '#061417', '#020506'],
        fogColor: 0x061417, fogDensity: 0.08,
        nebula: [
        ...
        starColor: 0xd8fff0, starOpacity: 0.8, starShape: 'crystal'
    },
    {
        id: 'ambar',
        label: 'Ámbar Cálido',
        bg: ['#2c1408', '#160a05', '#080302'],
        fogColor: 0x160a05, fogDensity: 0.085,
        nebula: [
        ...
        starColor: 0xffe0b0, starOpacity: 0.78, starShape: 'soft'
    },
    {
        id: 'monocromo',
    ...
    },
    {
        ...
        starColor: 0xffffff, starOpacity: 0.9, starShape: 'crystal'
    }
];
...
function wireBackgroundSelector() {
    ...
    function goToBgPreset(index) {
        bgPresetIndex = ((index % BACKGROUND_PRESETS.length) + BACKGROUND_PRESETS.length) % BACKGROUND_PRESETS.length;
...
function sampleGroupSurfacePoints(group, count) {
    ...
    group.traverse((node) => {
        ...
        if (bucketArea > 0) {
            ...
            buckets.push({ pos, matrix: node.matrixWorld.clone(), areas, triCount, cumulative: totalArea });
        }
    });
    ...
    for (let i = 0; i < count; i++) {
        ...
        for (let bi = 0; bi < buckets.length; bi++) {
            if (r <= buckets[bi].cumulative) { bucket = buckets[bi]; break; }
        }
        const localR = Math.random() * bucket.areas[bucket.triCount - 1];
        let lo = 0, hi = bucket.triCount - 1;
        while (lo < hi) {
            const mid = (lo + hi) >> 1;
            if (bucket.areas[mid] < localR) lo = mid + 1; else hi = mid;
        }
...
function loadParticleObjectFromGLB(url) {
    ...
    glbLoader.load(
        ...
        undefined,
        (error) => {
            console.warn('No se pudo cargar el .glb de particulas, se mantiene la figura de prueba:', error);
        }
    );
}
...
infoCardObjects.contact.element.addEventListener('mouseenter', () => { hoveringContactCard = true; });
infoCardObjects.contact.element.addEventListener('mouseleave', () => { hoveringContactCard = false; });
...
const categoryCardBaseRotY = {};

let portfolioData = null;

const GALLERY_TITLE_KEYS = {
    '3d': 'gallery_title_3d',
...
function galleryTitleForCategory(category) {
    const t = translations[currentLang];
    const key = GALLERY_TITLE_KEYS[category];
    return (key && t[key]) ? t[key] : portfolioData?.[category]?.title;
}
...
const CARD_CATEGORIES = [
    ...
    { key: 'diseno', titleKey: 'card_title_diseno', subKey: 'card_sub_diseno', badge: '02', slot: 0 },
    { key: 'edicion',titleKey: 'card_title_edicion',subKey: 'card_sub_edicion',badge: '03', slot: 1 }
];
...
function buildCardElement(catSpec) {
    ...
    el.innerHTML = `
        <div class="pc-inner">
            <img class="pc-img pc-active" src="${portfolioData[catSpec.key].projects[0].img}" alt="" draggable="false">
            <img class="pc-img" src="${portfolioData[catSpec.key].projects[1] ? portfolioData[catSpec.key].projects[1].img : portfolioData[catSpec.key].projects[0].img}" alt="" draggable="false">
    ...
    const covers = portfolioData[catSpec.key].projects.map(p => p.img);
...
function updateCardsLanguage() {
    ...
    projectCardInstances.forEach(inst => {
        ...
        if (titleEl) titleEl.innerHTML = `${t[inst.catSpec.titleKey]}<span class="pc-sub">${t[inst.catSpec.subKey]}</span>`;
    });
}
...
CARD_CATEGORIES.forEach(catSpec => {
    ...
    const hitMesh = new THREE.Mesh(new THREE.PlaneGeometry(CARD_HIT_WIDTH, CARD_HIT_HEIGHT), hitMat);
...
function layoutCategoryCardsMobile() {
    ...
    CARD_CATEGORIES.forEach((catSpec, i) => {
        ...
        const rotY = THREE.MathUtils.clamp(-offset * CATEGORY_ROTATION_STEP_MOBILE, -CATEGORY_MAX_ROTATION_MOBILE, CATEGORY_MAX_ROTATION_MOBILE);
        categoryCardBaseRotY[catSpec.key] = rotY;

        const obj = cssCardGroups[catSpec.key];
        if (obj) {
            obj.position.x = x;
...
function applyResponsiveProjectCards() {
    ...
    CARD_CATEGORIES.forEach(catSpec => {
        ...
        const obj = cssCardGroups[catSpec.key];
        if (obj) {
            obj.scale.set(PROJECT_CARD_SCALE_DESKTOP, PROJECT_CARD_SCALE_DESKTOP, PROJECT_CARD_SCALE_DESKTOP);
            obj.position.x = x;
            obj.position.z = -42;
...
function updateProjectCardsHover(clientX, clientY) {
    ...
    CARD_CATEGORIES.forEach(catSpec => {
        ...
        const isOver = r.width > 0 && clientX >= r.left && clientX <= r.right && clientY >= r.top && clientY <= r.bottom;
...
function rebuildGalleryHitMeshes(projects) {
    ...
    galleryHitMeshes = projects.map((p, i) => {
        ...
        const mesh = new THREE.Mesh(new THREE.PlaneGeometry(GALLERY_HIT_WIDTH, GALLERY_HIT_HEIGHT), mat);
...
function layoutGalleryCards() {
    ...
    galleryCardObjects.forEach((card, i) => {
        ...
        const rotY = THREE.MathUtils.clamp(-offset * GALLERY_ROTATION_STEP, -GALLERY_MAX_ROTATION, GALLERY_MAX_ROTATION);
...
function openGallery(category) {
    ...
    galleryOpen = true;
    document.getElementById('gallery-title').innerText = galleryTitleForCategory(category);
    document.getElementById('gallery-hud').classList.add('active');
    updateCategorySwitchVisibility();
}
...
function closeGallery() {
    ...
    galleryOpen = false;
    document.getElementById('gallery-hud').classList.remove('active');
    clearGalleryHover();
    galleryCardObjects.forEach(c => galleryGroup.remove(c.obj));
    galleryCardObjects = [];
    galleryHitMeshes.forEach(m => galleryHitGroup.remove(m));
...
if (galleryPrevBtn) {
    ...
    galleryPrevBtn.addEventListener('mouseenter', () => { galleryArrowHovered = true; });
    galleryPrevBtn.addEventListener('mouseleave', () => { galleryArrowHovered = false; });
}
if (galleryNextBtn) {
    galleryNextBtn.addEventListener('click', () => goToGalleryIndex(galleryCenterIndex + 1));
    galleryNextBtn.addEventListener('mouseenter', () => { galleryArrowHovered = true; });
    galleryNextBtn.addEventListener('mouseleave', () => { galleryArrowHovered = false; });
}
...
function handleGalleryWheel(e) {
    ...
    setTimeout(() => { galleryWheelLock = false; }, 260);
}
...
window.addEventListener('touchend', (e) => {
    ...
    const endX = (e.changedTouches && e.changedTouches[0]) ? e.changedTouches[0].clientX : horizTouchStartX;
...
function goToCategoryIndex(index) {
    categoryCenterIndex = ((index % CARD_CATEGORIES.length) + CARD_CATEGORIES.length) % CARD_CATEGORIES.length;
    layoutCategoryCardsMobile();
    playSFX('tick');
}
...
function updateCategorySwitchVisibility() {
    ...
    if (!nav) return;
    const show = isMobileViewport() && experienceStarted && currentSectionIndex === 2 && !galleryOpen;
...
if (catPrevBtn) catPrevBtn.addEventListener('click', () => goToCategoryIndex(categoryCenterIndex - 1));
if (catNextBtn) catNextBtn.addEventListener('click', () => goToCategoryIndex(categoryCenterIndex + 1));
...
window.openLightbox = function(cat, id) {
...
glowSlider.addEventListener('input', (e) => {
    document.documentElement.style.setProperty('--card-glow', e.target.value);
});
...
window.addEventListener('resize', () => {
    ...
    css3dRenderer.setSize(window.innerWidth, window.innerHeight);
    if (particleMaterial) particleMaterial.uniforms.uPixelRatio.value = isMobileViewport() ? 1 : Math.min(window.devicePixelRatio, 2);
    updateHeroLayout();
    applyResponsiveProjectCards();
    applyResponsiveInfoCards();
...
function animate() {
    ...
    if (experienceStarted) {
        ...
        const heroAlpha = computeAlphaForDistance(camera.position.z, HERO_OBJECT_Z, HERO_FADE_FULL_DIST, HERO_FADE_ZERO_DIST);
        ...
        const projAlpha = galleryOpen ? 0 : computeAlphaForDistance(camera.position.z, PROJECTS_OBJECT_Z, CARDS_FADE_FULL_DIST, CARDS_FADE_ZERO_DIST);
        CARD_CATEGORIES.forEach((catSpec, i) => {
            const obj = cssCardGroups[catSpec.key];
            if (!obj) return;
        ...
        const aboutAlpha = computeAlphaForDistance(camera.position.z, ABOUT_OBJECT_Z, INFO_CARD_FADE_FULL_DIST, INFO_CARD_FADE_ZERO_DIST);
        const contactAlpha = computeAlphaForDistance(camera.position.z, CONTACT_OBJECT_Z, INFO_CARD_FADE_FULL_DIST, INFO_CARD_FADE_ZERO_DIST);
